...
PL_Tableau.prototype.reset = function(clock, scope) {
	...
	this.m.exp = function(){return function(i,j){return m[i][j];}};
}
...
function PL_Spy(name, properties, pins) {
	...
	this.agentcount = false;
	if (pins != null) {
		if ("source" in pins) this.sourceName = pins.source;
		if ("inpt" in pins) {
			if (pins.inpt == "AData") this.agentcount = true;
			else this.inpt.exp = (("sourceName" in this) ? "this."+this.sourceName+"." : "") + pins.inpt;
		}
	}
	this.fstTime = true;
	this.properties = properties;
	...
	if (properties.vizid) {
		this.vinit.exp = (properties.init == null) ?
				((pins != null && "inpt" in pins) ? this.inpt.exp: 0) : properties.init;
...
PL_Spy.prototype.reset = function(clock, scope) {
	...
	if (this.vis) {
		if (this.agentcount) {
			this.svalue = this.ssource.Agentcount;
		} else {
			this.vinit.reset();
			this.svalue = this.vis.value =
				(typeof this.scope.vinit == "function") ? this.scope.vinit() : this.scope.vinit;
		}
	}
...
PL_Spy.prototype.strobe = function(t, scope) {
	pl_proto.prototype.strobe.call(this, t, scope);
	this.svalue = (this.agentcount) ? this.ssource.Agentcount :
				(typeof this.scope.inpt == "function") ? this.scope.inpt() : this.scope.inpt;
...
function PL_Scatterchart(name, props, pins) {
	...
	if (!("fill" in props)) props.fill = false;
	if (!("opacity" in props)) props.opacity = false;
	//debugger;
	this.scatterchart = scatterchart("#"+props.vizid, props.width, props.height,
			props.xLabel, props.yLabel, props.margin, props.radius, props.caption, props.toolTip, transition, props.fill, props.opacity);
...
PL_Table.prototype.reset = function(clock, scope) {
	...
	for (var i in this.props.keys) {
		var val = (Array.isArray(this.scope.vinit)) ? this.scope.vinit[i] : this.scope.vinit;
...
function PL_AgentviewerX(name, props, pins) {
	...
	if (pins != null) {
		this.pins = pins;
		if ("source" in pins) this.sourceName = pins.source;
		else if ("cellsource" in pins && "agentsource" in pins && pins.cellsource == pins.agentsource) {
			this.sourceName = pins.cellsource;
		} else if ("cellsource" in pins) {
			this.sourceName = pins.cellsource;
		} else if ("agentsource" in pins) {
			this.sourceName = pins.agentsource;
		}
	} else this.pins = {};
	this.props = props;
	if ("images" in props) this.images = props.images;
	...
	this.radius = props.radius || 5;
	this.aSizeOut = this.pins.aSizeOut || 1;
	...
	this.acscale = 1;
	this.ccscale = 1;
	this.agentSizing = props.agentSizing;
	this.agentColorAssigned = props.agentColorAssigned;
	this.loAgentColor = ("loAgentColor" in props) ? props.loAgentColor : 0;
	if (this.agentColorAssigned && "hiAgentColor" in props) {
		this.acscale = (props.colors.agent.length-1)/(props.hiAgentColor-this.loAgentColor);
	}
	if ("numCellColors" in props) {
		props.colors.cell.splice(props.numCellColors, props.colors.cell.length-props.numCellColors);
	}
	this.loCellColor = ("loCellColor" in props) ? props.loCellColor : 0;
	if ("hiCellColor" in props) {
		this.ccscale = (props.colors.cell.length-1)/(props.hiCellColor-this.loCellColor);
	}
	this.sscale = function(x){return x;}
	if (this.agentSizing && this.pins.aSizeOut != null) {
...
PL_AgentviewerX.prototype.initA = function() {
	var ans = new Array();
	if (this.agentvector == null) return ans;
	var outPin = this.pins.aColorOut;
	var sizePin = this.aSizeOut;
	var radius = this.radius;
	var sscale = this.sscale;
	var acscale = this.acscale;
	var ccscale = this.ccscale;
	var loAgentColor = this.loAgentColor;
	for (var i = 0; i < this.agentvector.count; i++) {
		var rec = this.agentvector.AGENTS(i);
		if (rec == null) continue;
		var rec1 = {id: rec.id, cx: rec.self._x_, cy: rec.self._y_, r: sscale(radius, rec[sizePin]), wrapped: rec.wrapped};
		if (this.agentColorAssigned) rec1.colorIdx = acscale*(rec[outPin]-loAgentColor);
		ans.push(rec1);
	}
	return ans;
};
...
PL_AgentviewerX.prototype.gleanA = function() {
	var ans = new Array();
	if (this.agentvector == null) return ans;
	var av = this.agentvector;
	var changes = av.changes._tot_;
	if (changes.length == 0) return ans;
	var outPin = this.pins.aColorOut;
	var radius = this.radius;
	var sizePin = this.aSizeOut;
	var sscale = this.sscale;
	...
	for (var id in changes) {
		var rec = changes[id];
		var rec1 = {id: rec.id, cx: rec.self._x_, cy: rec.self._y_, r: sscale(radius, rec[sizePin]), wrapped: rec.wrapped};
		if (that.agentColorAssigned) rec1.colorIdx = acscale*(rec[outPin]-loAgentColor);
		ans.push(rec1);
	}
	return ans;
};
...
PL_AgentviewerX.prototype.initC = function() {
	var ans = new Array();
	if (this.cellmatrix == null) return ans;
	var outPin = this.pins.cColorOut;
	var ccscale = this.ccscale;
	var loCellColor = this.loCellColor;
	for (var i = 0; i < this.rows; i++)
		for (var j = 0; j < this.cols; j++) {
			var rec = this.cellmatrix.CELLS(i,j);
			ans.push({id: rec.id, r: rec.row, c: rec.col, colorIdx: ccscale*(rec[outPin]-loCellColor)});
		}
	return ans;
};
...
PL_AgentviewerX.prototype.gleanC = function() {
	...
	if (this.cellmatrix == null) return ans;
	var outPin = this.pins.cColorOut;
	var cm = this.cellmatrix;
	var changes = cm.changes._tot_; // [outPin];
	if (changes.length == 0) return ans;
	var ccscale = this.ccscale;
	var loCellColor = this.loCellColor;
	for (var id in changes) {
...
PL_AgentviewerX.prototype.prereset = function(clock) {
	if (this.fstTime) {
		this.source =
			(typeof this.sourceName == "function") ? this.sourceName.call(this.cap)
				: this.cap[this.sourceName];
		this.cellmatrix = (this.source.isKind("CellMatrix")) ? this.source :
			(this.source.isKind("Simworld")) ? this.source.cellmatrix : null;
		this.agentvector = (this.source.isKind("AgentVector")) ? this.source :
			(this.source.isKind("Simworld")) ? this.source.agentvector : null;
	}
	if (!this.interactive) {
		this.fstTime = false;
		return;
	...
	if (this.fstTime) {
		...
		if (this.cellmatrix != null) this.cellmatrix[this.pins.cColorIn] = function() {
			var ans = 0;
			var rec = that.cBucket[this.id];
			if (rec) ans = rec.colorIdx;
			return ans;
		}
		if (this.agentvector != null) this.agentvector[this.pins.aColorIn] = function () {
			var ans = 0;
	...
	this.agentvector.regenerate(this.aBucket);
}
...
PL_AgentviewerX.prototype.strobe = function(t, scope) {
	...
	if (this.agentvector != null) {
		var births = this.agentvector.changes["_births_"].map(function(id){
			var rec = source.agentvector.AGENTS(id);
			return {id: rec.id, cx: rec._x_, cy: rec._y_};
		});
		var deaths = this.agentvector.changes["_deaths_"];
		this.av.create(births);
		this.av.kill(deaths);
	}
...
PL_Tabulator.prototype.reset = function(clock, scope) {
	...
	case "AgentVector":
		var av = this.source;
		this.State = new Object();
		for (var i = 0; i < av.vector.length; i++) {
			var rec = av.AGENTS(i);
			if (rec == null) continue;
			var n = rec[pin];
			this.State[i] = n;
