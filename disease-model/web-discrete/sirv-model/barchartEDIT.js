...
//		}
	...
	function show() {
		...
		.attr("fill", function(d, i) {
			return (col) ? col[i] : "steelblue";
		})
		/*.attr("y", function(d){
			return (d.y >= 0) ? (Height) - (y(d.y)) : (Height) - y(0);
		})
		.attr("height", function(d, i){
			return (d.y >= 0) ? y(d.y) : y(-d.y);
		});*/
		bar.select("rect")
		.transition()
		.attr("y", function(d) {
			var yOff = (d.y >= 0) ? d.y : 0;
			var tmpH = height - (margin.top);
			return height - (yOff * 0.2);
			//return height - yOff;
			//return 0;
			//return Height - yOff;
			//return Height;
		...
		.attr("height", function(d) {
			var yOff = (d.y >= 0) ? d.y : 0;
			var tmpH = height - (margin.top + margin.bottom);
			//return yOff;
			//return tmpH;
			return (yOff * 0.2);
		})
